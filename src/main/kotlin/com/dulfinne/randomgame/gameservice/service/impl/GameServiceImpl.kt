...
class GameServiceImpl(
  ...
  override suspend fun getAllGames(request: PaginationRequest): List<GameResponse> =
    gameSortingRepository.findAllBy(
      PageRequest.of(
        request.offset,
        request.limit
      )
  ...
  override suspend fun getAllGamesByUsername(
    ...
    gameSortingRepository.findAllByUsername(
      username,
  ...
  override suspend fun createGame(
    ...
    val balance = clientService.getUserBalance(username).balance
    validateBidAmount(balance, request.bid)
  ...
  private suspend fun getGameIfExists(gameId: String): Game {
    return gameRepository.findById(gameId)
      ?: throw EntityNotFoundException(
        ExceptionKeys.GAME_NOT_FOUND.format(
          gameId
        )
      )
  }
  ...
  private fun validateBidAmount(balance: BigDecimal, bid: BigDecimal) {
    if (balance < bid)
      throw ActionNotAllowedException(
        ExceptionKeys.NOT_ENOUGH_MONEY.format(
          bid
        )
      )
  }
