...
class GlobalExceptionHandler {
  ...
  suspend fun handleValidationExceptions(ex: WebExchangeBindException): Map<String, String> {
    return ex.bindingResult.fieldErrors.associate { fieldError: FieldError ->
      fieldError.field to (fieldError.defaultMessage
        ?: ValidationKeys.DEFAULT_MESSAGE)
    }
  }
  ...
  suspend fun handleClientException(ex: ClientException): ResponseEntity<ErrorResponse> {
    val status = HttpStatus.valueOf(ex.errorCode)
    return ResponseEntity.status(status)
      .body(ErrorResponse(status, ex.message))
  }
